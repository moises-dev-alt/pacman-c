...
static const char *raw_map_classic[MAP_H] = {
    "############################",
    "#............##............#",
    "#.####.#####.##.#####.####.#",
    "#o####.#####.##.#####.####o#",
...
static const char *raw_map_shadow[MAP_H] = {
    "############################",
    "#............##............#",
    "#.####.#####.##.#####.####.#",
    "#o####.##.##.##.##.##.####o#",
...
void model_init(GameModel *m) {
    ...
    m->shield_timer = 0;
    m->speed_timer = 0;
    m->ghost_slow_timer = 0;
    m->time_freeze_timer = 0;
    ...
    m->powerup_tree = avl_insert(m->powerup_tree, 1, 0);  /* Escudo */
    m->powerup_tree = avl_insert(m->powerup_tree, 2, 0);  /* Turbo */
    m->powerup_tree = avl_insert(m->powerup_tree, 3, 0);  /* Fantasmas lentos */
    m->powerup_tree = avl_insert(m->powerup_tree, 4, 0);  /* Congela tempo */
    m->powerup_tree = avl_insert(m->powerup_tree, 5, 0);  /* Pontos em dobro */
}
...
void model_load_map(GameModel *m) {
    ...
    const char **source = (m->map_variant == 1) ? raw_map_shadow : raw_map_classic;
...
void model_activate_shield(GameModel *m) {
    m->shield_timer = SHIELD_TIME;
}

void model_activate_speed(GameModel *m) {
    m->speed_timer = SPEED_TIME;
}

void model_activate_slow_ghosts(GameModel *m) {
    m->ghost_slow_timer = GHOST_SLOW_TIME;
}

void model_activate_time_freeze(GameModel *m) {
    m->time_freeze_timer = TIME_FREEZE_TIME;
}

void model_activate_double_points(GameModel *m) {
    m->double_score_timer = DOUBLE_TIME;
}

void model_switch_map(GameModel *m, int variant) {
    m->map_variant = variant ? 1 : 0;
    model_load_map(m);
}

void model_next_phase(GameModel *m) {
    m->level++;
    m->map_variant = (m->level - 1) % 2;
    if (m->lives < START_LIVES) m->lives++;
    if (m->level % 2 == 0) m->speed_timer += 30;
    if (m->level % 3 == 0) m->ghost_slow_timer += 30;
    model_switch_map(m, m->map_variant);
}
...
void model_move_pacman(GameModel *m) {
    ...
    if (m->grid[m->pacman.y][m->pacman.x] == TILE_PELLET) {
        ...
        m->score += (m->double_score_timer > 0) ? 20 : 10;
        m->pellets_left--;
        ...
        m->grid[m->pacman.y][m->pacman.x] = TILE_EMPTY;
        m->score += (m->double_score_timer > 0) ? 100 : 50;
        m->pellets_left--;
        ...
        /* Coleta power-up na AVL (incrementa quantidade) e aplica efeito extra */
        int pu_type = (rand() % 5) + 1;
        if (pu_type == 1) {
            model_activate_shield(m);
        } else if (pu_type == 2) {
            model_activate_speed(m);
        } else if (pu_type == 3) {
            model_activate_slow_ghosts(m);
        } else if (pu_type == 4) {
            model_activate_time_freeze(m);
        } else {
            model_activate_double_points(m);
        }
        m->powerup_tree = avl_insert(m->powerup_tree, pu_type, 1);
    }
...
void model_move_ghosts(GameModel *m) {
    ...
    for (int idx = 0; idx < GHOSTS; idx++) {
        ...
        /* Congela tempo: fantasmas não se movem */
        if (m->time_freeze_timer > 0) continue;

        /* Vulneraveis ou power slow deixam os fantasmas mais lentos */
        if ((g->vulnerable && m->frame_count % 2 != 0) ||
            (m->ghost_slow_timer > 0 && m->frame_count % 2 != 0)) continue;
    ...
    for (int i = 0; i < GHOSTS; i++) {
        if (m->ghosts[i].e.x == m->pacman.x && m->ghosts[i].e.y == m->pacman.y) {
            if (m->ghosts[i].vulnerable) {
                m->ghost_eat_combo++;
                int base = 200 * m->ghost_eat_combo;
                int bonus = (m->double_score_timer > 0) ? base : 0;
                m->score += base + bonus;
                if (m->score > m->high_score) m->high_score = m->score;
                ...
                return base + bonus;  /* Retorna bonus para exibir */
            } else if (m->shield_timer > 0) {
                m->shield_timer = 0;
                m->ghosts[i].e.x = m->ghosts[i].e.start_x;
                m->ghosts[i].e.y = m->ghosts[i].e.start_y;
                m->ghosts[i].in_house = 1;
                m->ghosts[i].release_timer = 20;
                return 0;
            } else {
                m->lives--;
                if (m->lives <= 0) {
